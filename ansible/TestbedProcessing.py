...
veos_file = "veos"
veos_vtb_file = "veos.vtb"
allCred_file = "group_vars/all/creds.yml"
TESTS_DIR = "/tests/"
...
backupList.append(veos_vtb_file)
backupList.append(allCred_file)
...
os.makedirs(args.backupdir + "/" + timestamp + "/group_vars/all")  # create group_vars/all folder under the timestamped folder
...
def makeMain(data, outfile):
    ...
    dictData = {
        ...
        "sonic_console_base": veos.get("sonic_console_base")
    }

    proxy_env = veos.get("proxy_env")
    ...
    try:
        with open(csv_file, "w") as f:
            ...
            for group, groupDetails in topology.items():
                ...
                ptf = groupDetails.get("ptf", 'ptf_'+groupName)
...
"""
makeVeosVtb(veos_file, lab_file, outfile)
@:parameter veos_file - reads from veos
@:parameter lab_file - reads from lab
@:parameter outfile - writes to veos.vtb
"""
def makeVeosVtb(veos_file, lab_file, outfile):
    with open(veos_file, "r") as veos, open(lab_file, "r") as lab, open(outfile, "w") as toWrite:
        toWrite.write(veos.read() + lab.read())
...
"""
makeAll_creds(data, outfile)
@:parameter data - the dictionary to look through
@:parameter outfile - the file to write to
Generate /group_vars/all/creds.yml
Works by looking through veos dictionary and look under eos_ansible/sonic_ansible

move eos/sonic creds into group_vars/all to share them among different roles
"""
def makeAllCreds(data, outfile):   
    veos = data
    sonic_ansible = veos.get("sonic_ansible")
    eos_ansible = veos.get("eos_ansible")

    result = {
        "eos_login": eos_ansible.get("eos_login"),
        "eos_password": eos_ansible.get("eos_password"),
        "eos_root_password": eos_ansible.get("eos_root_password"),
        "eos_default_login": eos_ansible.get("eos_default_login"),
        "eos_default_password": eos_ansible.get("eos_default_password"),
        "sonic_login": sonic_ansible.get("sonic_login"),
        "sonic_default_passwords": sonic_ansible.get("sonic_default_passwords"),
        "sonic_password": sonic_ansible.get("sonic_password")
    }
    with open(outfile, "w") as toWrite:
        toWrite.write("---\n")
        yaml.dump(result, stream=toWrite, default_flow_style=False)
...
def updateDockerRegistry(docker_registry, outfile):
    if not docker_registry.get("docker_registry_host"):
        ...
        with open(outfile, "w") as toWrite:
            ...
            toWrite.write("\n\n")
            toWrite.write("docker_registry_username: " + (docker_registry.get("docker_registry_username") or "") + "\n")
            toWrite.write("docker_registry_password: " + (docker_registry.get("docker_registry_password") or "") )
...
def main():
    ...
    copyfile(args.basedir + testbed_file, os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir)) + TESTS_DIR + testbed_file)
    ...
    print("\tCREATING All/CREDS: " + args.basedir + allCred_file)
    makeAllCreds(veos, args.basedir + allCred_file)  # Generate eos\creds.yml (CREDS)
    ...
    print("\tCREATING VEOSVTB FILE: " + args.basedir + veos_vtb_file)
    makeVeosVtb(veos_file, lab_file, args.basedir + veos_vtb_file)
    copyfile(args.basedir + veos_vtb_file, os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir)) + TESTS_DIR + veos_vtb_file)
