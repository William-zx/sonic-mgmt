...
def parse_xml(filename, hostname):
    ...
    if hwsku == "Force10-S6000":
        ...
        for i in range(1, 33):
            port_alias_to_name_map["Ethernet%d/1" % i] = "Ethernet%d" % ((i - 1) * 4)
    elif hwsku == "INGRASYS-S9100-C32" or hwsku == "INGRASYS-S8810-32Q":
        for i in range(1, 33):
            port_alias_to_name_map["Ethernet%d/1" % i] = "Ethernet%d" % ((i - 1) * 4)
    ...
    elif hwsku == "Cig-CS6436-56P":
        for i in range(1, 33):
            port_alias_to_name_map["Ethernet%d/1" % i] = "Ethernet%d" % (i - 1)
    elif hwsku == "Accton-AS7116-54X":
        for i in range(1, 33):
            port_alias_to_name_map["Ethernet%d/1" % i] = "Ethernet%d" % (i - 1)
    elif hwsku == "INGRASYS-S9130-32X" or hwsku == "INGRASYS-S9230-64X":
        for i in range(1, 33):
            port_alias_to_name_map["Ethernet%d/%d" % ((i+3)//4, ((i-1) % 4 + 1))] = "Ethernet%d" % (i - 1)
