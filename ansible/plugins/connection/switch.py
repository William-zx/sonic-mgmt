    ...
    def _spawn_connect(self):
        ...
        while attempt < len(self.login['user']):
            ...
            if user != last_user:
                ...
                if i == 1:
                    if connection_retry > self.connection_max_retry:
                        raise AnsibleError("Connect to server failed!! Retried %d times!" % self.connection_max_retry)
                    connection_retry += 1
        ...
        if 'Arista' in client.before:
            ...
            self.sku = 'nxos'
        elif ('MLNX-OS' in client.before) or ('Onyx' in client.before) or ("Digital China" in client.before):
            self.sku = 'mlnx_os'
