...
function add_topo
{
  ...
  ansible-playbook -i lab -l "$dut" roles/vm_set/files/tools/topo_marker.yml -e action="add" -e topo="$topo" $@
...
function remove_topo
{
  ...
  ansible-playbook -i lab -l "$dut" roles/vm_set/files/tools/topo_marker.yml -e action="remove" -e topo="$topo" $@

  # flush ansible cache
  rm ~/.ansible/cache/"$dut" || true
