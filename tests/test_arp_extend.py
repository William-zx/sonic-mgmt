...
g_vars          = {}
test_ip         = ["20.1.1.1/8", "30.1.1.1/8"]
test_route      = "40.1.1.0/24"
...
def setup_init(mg_facts, duthost, ptfhost):
    ...
    duthost.shell("mkdir -p /tmp/arp_extend")

    # copy ptftest script
    ptfhost.copy(src="ptftests", dest="/root")
    ptfhost.shell("mkdir -p /tmp/arp_extend")
...
class TestCase1_DynamicArp():
    ...
    def test_dynamic_arp_on_port_intf(self, ptfhost, duthost, setup_intf):
        for ip in test_ip:
            ...
            res = duthost.shell("show arp {}".format(ptf_ip))["stdout"]
            assert str(ptf_ip) in res, "Dynamic arp {} must in arp table".format(ptf_ip)
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[expect_true].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))
        ...
        for ip in test_ip:
            ...
            res = duthost.shell("show arp {}".format(ptf_ip))["stdout"]
            assert str(ptf_ip) not in res, "Dynamic arp {} must not in arp table".format(ptf_ip)
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[expect_false].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))
...
class TestCase2_StaticArp():
    ...
    def test_static_arp_on_port_intf(self, ptfhost, duthost, setup_intf):
        for ip in test_ip:
            ...
            res = duthost.shell("show arp {}".format(ptf_ip))["stdout"]
            assert str(ptf_ip) in res, "Static arp {} must in arp table".format(ptf_ip)
        ...
        for ip in test_ip:
            ...
            res = duthost.shell("show arp {}".format(ptf_ip))["stdout"]
            assert str(ptf_ip) not in res, "Static arp {} must not in arp table".format(ptf_ip)
...
class TestCase3_AddAndFlushArpEntryWhichIsRouteNexthop():
    ...
    def test_on_port_intf(self, duthost, ptfhost, setup_route):
        ...
        res = duthost.shell("show ip route {}".format(test_route))["stdout"]
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[nexthop_add].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))

        duthost.shell("sonic-clear arp")
        time.sleep(SLEEP_TIME)

        res = duthost.shell("show arp {}".format(g_vars["route_nexthop"]))["stdout"]
        assert str(g_vars["route_nexthop"]) not in res, "the arp {} must not in arp table".format(g_vars["route_nexthop"])
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[nexthop_flush].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))
...
class TestCase4_MacPortChangeOfArp():
    @pytest.fixture(scope="function", autouse=True)
    ...
    def test_on_port_intf(self, duthost, ptfhost, setup_route):
        ...
        res = duthost.shell("show arp {}".format(g_vars["route_nexthop"]))["stdout"]
        assert str(g_vars["route_nexthop"]) in res and host_mac in res, "the arp {} mac {} must in arp table".format(g_vars["route_nexthop"], host_mac)
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[nexthop_mac_change].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))
    ...
    def test_on_vlan_intf(self, duthost, ptfhost, setup_route):
        ...
        res = duthost.shell("show arp {}".format(g_vars["route_nexthop"]))["stdout"]
        assert str(g_vars["route_nexthop"]) in res and g_vars["vlan_member_port_2"] in res, "the arp {} port {} must in arp table".format(g_vars["route_nexthop"], g_vars["vlan_member_port_2"])
        ...
        ptf_runner( ptfhost, \
                    ...
                    log_file="/tmp/arp_extend/[{}]_[{}]_[nexthop_port_change].log".format(self.__class__.__name__, sys._getframe().f_code.co_name))
